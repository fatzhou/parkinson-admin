<template>
  <div class="list-wrap" v-show="list && list.length > 0">
    <div class="list-content" @click="goDetail(item.id)" v-show="!isShowDetails" v-for="(item,index) in list">
      <div class="item">
        <div class="item-title clear">
          ...
          <span class="text-enter f-r" 
                :class="{'active': item.status == 0, 'default': item.status == 2, 'cancel': item.status == 1}">
                {{statusDesc[item.status]}}
          </span>
        </div>
        <div class="item-con clear">
          <div class="icon-enter f-l" :class="{'active': item.status != 2, 'default': item.status == 2}"></div>
          <span><strong class="yellow">{{item.applyCount}}</strong>/{{item.containCount}}</span>
          <span class="f-r">{{item.deadlineAt}}</span>
        </div>
      </div>
    </div>
  </div>
